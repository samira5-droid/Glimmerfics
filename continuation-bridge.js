(() => {
  ...
  function advance(b,e){
    ...
    let s;try{s=JSON.parse(localStorage.getItem(KEY)||'null')}catch(_){return true}
    if(!s)return true;
    const target=b.dataset.continuationTarget;if(!target)return false;
    let d={};try{d=JSON.parse(b.dataset.delta||'{}')}catch(_){d=deltas[target]||{}}
    s.history=Array.isArray(s.history)?s.history:[];
    const label=b.innerText.replace(/^\s*\d+\s*/,'').trim();
    s.history.push({from:s.node,label,to:target,at:Date.now()});
    s.sebastian=(s.sebastian||0)+(d.sebastian||0);s.ominis=(s.ominis||0)+(d.ominis||0);s.anne=(s.anne||0)+(d.anne||0);s.node=target;
    localStorage.setItem(KEY,JSON.stringify(s));location.reload();return true;
  }
  document.addEventListener('pointerdown',e=>{const b=e.target.closest('#choices button[data-continuation-target]');if(b)advance(b,e)},true);
})();
